...
// Generates a random string, stores it in sessionStorage, and returns it.
// The OAuth callback verifies the returned state matches before accepting the token.
...
function verifyState(returnedState) {
  ...
  if (!expected || expected !== returnedState) {
    ...
    setSyncStatus("Sign-in failed: security check did not pass ✗", "err");
    return false;
  }
  return true;
}
...
function initGIS() {
  ...
  tokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: async (resp) => {
      ...
      if (!verifyState(resp.state)) return;
...
async function googleSignIn() {
  ...
  // Generate and pass a fresh state value with every sign-in attempt
  tokenClient.requestAccessToken({ prompt: promptMode, state: generateState() });
}
